...
class PurchaseManage {
  ...
  getByMonth(month = "02") {
    console.log(this.data.filter((data) => new Date(data.created_at).getMonth() == parseInt(month) - 1))
  }

  getPriceByPerson(name) {
    console.log(
      this.data
        .filter((value) => value.customer.name == name)
        .map((value) => value.items.map((item) => item.qty * item.price).reduce((x, y) => x + y))
        .reduce((x, y) => x + y)
    );
  }
  ...
  getPriceLowerThan(lower) {
   console.log([
      ...new Set(
        data
          .filter((value) => value.items.map((item) => item.qty * item.price).reduce((x, y) => x + y) < lower)
          .map((value) => value.customer.name)
      ),
    ])
}
}

const purchase = new PurchaseManage(data);
purchase.getByMonth("03");
