...
export default new Vuex.Store({
  ...
  actions: {
    ...
    searchItems({ commit }, payload) {
      payload.length >= 3
        ? commit("setFoundItems", payload)
        : commit("setFoundItems", "");
    },
  ...
  mutations: {
    totalCount(state, payload) {
      ...
      if (payload.increase) state.items[itemIndex].total += 1;
      else {
        state.items[itemIndex].total -= 1;
        if (state.items[itemIndex].total < 0) state.items[itemIndex].total = 0;
      }
    ...
    setItems(state, items) {
      ...
      if (cache) state.originalItems = JSON.parse(cache);
        ...
        state.originalItems = items.map((v) => ({
          ...v,
          price: getRandomInt(18000, 100000),
          selected: false,
          total: 0,
        }));
