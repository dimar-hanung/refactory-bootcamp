...
export default {
  ...
  methods: {
    setPage() {
      console.log(this.$refs.foto.currentPage);
      const params = new URLSearchParams(location.search);
      params.set("page", this.$refs.foto.currentPage+1);
  ...
  async created() {
    await console.log(this.$route.$refs);
    if (this.$refs.foto ** this.$route.query.page) {
      this.$refs.foto.goToPage(parseInt(this.$route.query.page));
    }
  },
};
</script>